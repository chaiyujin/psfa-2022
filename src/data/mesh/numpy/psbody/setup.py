...
class build_deflate_cgal(Command):
    ...
    user_options = [
        (
            "cgal-location=",
            None,
            "specifies the location of the cgal archive (tar.gz file)",
        ),
    ]
...
class build_ext(_build_ext):
    ...
    user_options = [
        (
            "boost-location=",
            None,
            "specifies the location of the boost folder (only include needed)",
        ),
    ] + _build_ext.user_options

    def initialize_options(self):
        self.boost_location = os.environ.get("BOOST_LOCATION")
...
package_dir = {
    namespace_package: "%s-mesh-namespace" % namespace_package,
    "%s.mesh" % namespace_package: "mesh",  # actual subpackage described here
}

setup(
    name="%s-mesh" % namespace_package,
    version="v0.0.1",
    packages=packages,
